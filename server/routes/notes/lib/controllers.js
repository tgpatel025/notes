...
const updateNote = async (req, res, next) => {
  try {
    const data = matchedData(req);
    const [updateCount] = await Notes.update(data, {
      where: { id: data.id },
      raw: true,
    });
    if (!updateCount)
      return res.json({
        message: "Oops, something went wrong. Please try to save again.",
      });
...
const shareNote = async (req, res, next) => {
  try {
    ...
    if (shareWith.includes(req.session.user.id))
      return res.json({ message: "User cannot share note with itself." });
    const dataToInsert = shareWith.map((userId) => ({
      noteId: id,
      sharedWithId: zuserId,
      sharedById: req.session.user.id,
    }));
    ...
    if (!shared.length)
      return res.json({
        message: "Oops, something went wrong. Please try to share again.",
      });
